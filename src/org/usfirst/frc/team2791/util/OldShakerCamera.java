...
public class OldShakerCamera implements Runnable {
	private static OldShakerCamera cameraInstance;
	...
	private OldShakerCamera(String camPort) {
	...
	public static OldShakerCamera getInstance() {
		if (cameraInstance == null)
			cameraInstance = new OldShakerCamera("cam2");
...
//						SmartDashboard.putNumber("Distance from target", getRange());
	...
	public double getRange() {
		...
		// Math.toRadians((-getNormalizedCenterOfMassX(getTarget().CenterOfMassY)
		// * CAMERA_FOV_VERTICAL / 2.0
		// + CAMERA_PITCH_DEG)));
		// SmartDashboard.putNumber("angle value",
		// (getNormalizedCenterOfMassX(getTarget().CenterOfMassY) *
		// CAMERA_FOV_VERTICAL / 2.0
		// + CAMERA_PITCH_DEG));
